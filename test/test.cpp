...
double partialwidth(double x, double E) //Formule de la largeur de desintegration partielle implementee pour etre utilisee avec TF1
{
    double par[7]; //masse fermion, KTr, hbar, c, alpha, Egamma, 1
    par[0] = 0.000511; par[1] = LIVParameter(E); par[2] = 1; par[3] = 1; par[4]=1.0/137; par[5] = E; par[6] = 1;
...
double integrate(double E, double a, double b)
{
    int numSteps = 1000;
    ...
    for (int i = 0; i < numSteps; i++) {
        ...
        double fx1 = partialwidth(x1, E);      // Valeur de la fonction au point de départ
        double fx2 = partialwidth(x2, E);      // Valeur de la fonction au point de fin
...
int main(){
    ...
    std::cout<<"Gamma is : "<<integrate(E, 1/2*(E-ebar), 1/2*(E+ebar))<<std::endl;
    return 0;
}